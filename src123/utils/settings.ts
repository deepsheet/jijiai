...
export async function loadSettings(): Promise<Settings> {
  try {
    // 1. 先尝试从 localStorage 加载（优先级最高）
    const stored = localStorage.getItem(SETTINGS_KEY);
    if (stored) {
      console.log('从 localStorage 加载设置');
      const localStorageSettings = JSON.parse(stored);
      
      // 2. 尝试从配置文件加载，合并到 localStorage 设置中
      try {
        const response = await fetch('/config.json');
        if (response.ok) {
          const configSettings = await response.json();
          console.log('从配置文件加载设置，合并到 localStorage 设置');
          // 合并配置，但保留 localStorage 中的设置（包括主题）
          return { ...configSettings, ...localStorageSettings };
        }
      } catch (e) {
        console.log('配置文件不存在或加载失败，使用 localStorage 设置');
      }
      
      return localStorageSettings;
    }
    
    // 3. 如果 localStorage 没有设置，尝试从配置文件加载
    try {
      const response = await fetch('/config.json');
      if (response.ok) {
        const configSettings = await response.json();
